...
class FileMaker(object):
    ...
    def presigned_put_object(self, project, suffix, expires=0):
        ...
        self.create_bucket_if_not_exists()
        return {
            'put': self.minio_cli.presigned_put_object(
                self.bucket_name, path,
                expires=datetime.timedelta(minutes=expires)
            ),
            'get': self.minio_cli.presigned_put_object(
                self.bucket_name, path,
                expires=datetime.timedelta(minutes=expires)
            )
        }

    def presigned_get_object(self, project, xobject, expires=0):
        if expires <= 0:
            expires = self.expires

        self.create_bucket_if_not_exists()
        return self.minio_cli.presigned_get_object(
            self.bucket_name, xobject,
            expires=datetime.timedelta(minutes=expires)
        )
